-- Run after feature_leave_enhancements.sql and feature_school_directory.sql.
-- school_id controls access; assigned_school_id is the current working school.
-- Keep the prior item-school reference when an AO first changes an assignment.
begin;

-- Include the shared helper for installations with an older account setup.
...
alter table public.leave_employees add column if not exists item_school_id text;
alter table public.leave_employees add column if not exists ao_assigned_school_id text;

create table if not exists public.leave_staff_assignment_log (
  id uuid primary key default gen_random_uuid(),
  employee_id uuid not null references public.leave_employees(id),
  previous_school_id text not null,
  new_school_id text not null,
  changed_by uuid not null,
  changed_at timestamptz not null default now()
);
alter table public.leave_staff_assignment_log enable row level security;
revoke all on public.leave_staff_assignment_log from anon, authenticated;
grant select on public.leave_staff_assignment_log to authenticated;
drop policy if exists assignment_log_hrmo on public.leave_staff_assignment_log;
create policy assignment_log_hrmo on public.leave_staff_assignment_log
  for select to authenticated using ((select public.lcms_is_hrmo()));

-- Protect assignments already made with the previous version of these functions.
update public.leave_employees e
set ao_assigned_school_id = latest.new_school_id
from (
  select distinct on (employee_id) employee_id, new_school_id
  from public.leave_staff_assignment_log
  order by employee_id, changed_at desc, id desc
) latest
where e.id = latest.employee_id and e.school_id = latest.new_school_id
  and latest.new_school_id <> 'UNASSIGNED' and e.ao_assigned_school_id is null;

-- Return only directory information, never leave balances or private identifiers.
-- The return shape adds assignment_locked, so replace the old signature first.
...
returns table(id uuid, last_name text, first_name text, middle_name text,
  "position" text, school_id text, item_number text, assignment_locked boolean)
...
begin
  ...
  return query select e.id, e.last_name, e.first_name, e.middle_name,
    e.position, e.school_id, e.item_number, e.ao_assigned_school_id is not null
...
revoke all on function public.lcms_assign_teacher_to_school(uuid, text, boolean) from public;
grant execute on function public.lcms_search_teachers_by_name(text, text, text) to authenticated;
grant execute on function public.lcms_assign_teacher_to_school(uuid, text, boolean) to authenticated;
-- Keep CSV linking compatible with assignment tracking and concurrent school uploads.
  ...
  for r in select * from jsonb_array_elements(rows) loop
    ...
    if match_count = 0 then
      ...
      update public.leave_employees set
        ...
        updated_at = now(), updated_by = actor
      where id = matched_id and school_id = 'UNASSIGNED' and ao_assigned_school_id is null;
      linked := found;
      if linked then
