-- Run after feature_admin_assignment_lock.sql and feature_school_head_designate.sql.
-- The AO self-service search/CSV-link path excluded every Head Teacher and
-- School Head-titled position outright. That was too broad: only an actual
-- Principal/Assistant Principal/School Head position, or a Head Teacher or
-- Master Teacher explicitly designated as acting School Head
-- (school_head_designate), is HRMO-managed. Ordinary Head Teachers and
...
begin
  ...
  if employee.is_active is distinct from true or employee.emp_type <> 'Teaching'
    or employee.position ~* 'principal|school head' or employee.school_head_designate then
